...
plot_ts_10th <- ggplot(ts_ex, aes(x = years, y = percent_diff)) + geom_point() + geom_line() +
  ...
  theme(legend.position = "none") +
  geom_hline(aes(yintercept = 0), linetype = "dashed") +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = quantile(ts_ex$percent_diff[1:33], probs = .05), ymax = quantile(ts_ex$percent_diff[1:33], probs = .95), fill = "lightgreen", alpha = .5) +
...
y <- ts %>% 
    ...
    mutate(change_5 = ifelse(percent_diff>5 | percent_diff<(-5), 1, 0),
           ...
           change_pct = ifelse(percent_diff>0 & percent_diff<quantile(percent_diff[1:33], probs = .05, na.rm=T), 1, 0),
           change_pct = ifelse(percent_diff>0 & percent_diff>quantile(percent_diff[1:33], probs = .95, na.rm=T), 1, 0)) %>% 
