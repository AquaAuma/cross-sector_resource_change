...
## Long-term gradual change and shock detection

Example dataset for agriculture maize crop yield for CROVER crop model, GFDL-ESM4, SSP 585, and for the United States.

1.  Long-term change with threshold
2.  Shock frequency
3.  End of century average
...
plot_ts_10th <- ggplot(ts_ex, aes(x = years, y = percent_diff)) + geom_point() + geom_line() +
  ...
  ggtitle("End of century average beyond 10th percentile") +
  theme(legend.position = "none") +
  geom_hline(aes(yintercept = 0), linetype = "dashed") +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = quantile(ts_ex$percent_diff[1:33], probs = .1), ymax = quantile(ts_ex$percent_diff[1:33], probs = .9), fill = "lightgreen", alpha = .5) +
...
To compare the effect of using the 10th percentile of the historic period, we look at the 10th percentile and the % difference by the end of the century. This plot is excluding outliers when the % difference is higher than 200 etc.
...
percentiles <- ts %>% 
  ...
  summarize(avg = mean(percent_diff[88:117], na.rm=T),
            pct = ifelse(avg<0, quantile(percent_diff[1:33], probs = .1, na.rm=T), NA),
            pct = ifelse(avg>0, quantile(percent_diff[1:33], probs = .9, na.rm=T), pct)) %>% 
...
ggplot(percentiles, aes(x = pct, y = avg, color = climates)) + geom_point(alpha = .5) +
  ...
  scale_colour_manual(values = c("purple1","purple4","lightskyblue","blue")) +
  theme_bw() +
...
y <- ts %>% 
    ...
    summarize(percent_diff = mean(percent_diff[88:117], na.rm=T)) %>% 
    mutate(change_5 = ifelse(percent_diff>5 | percent_diff<(-5), 1, 0),
           ...
           change_pct = ifelse(percent_diff>0 & percent_diff<quantile(percent_diff[1:33], probs = .1, na.rm=T), 1, 0),
           change_pct = ifelse(percent_diff>0 & percent_diff>quantile(percent_diff[1:33], probs = .9, na.rm=T), 1, 0)) %>% 
...
ggplot(y, aes(x = threshold, y = probas, fill = climates)) + geom_boxplot() +
  facet_wrap(~n_res) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = -45, hjust=0)) +
  scale_fill_manual(values = c("purple1","purple4","lightskyblue","blue")) +
  scale_x_discrete(limits = c("5","10","25","pct"))
